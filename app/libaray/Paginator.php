...
class Paginator {
    ...
    $this->_conn = connection::getInstance(); //mysql connection resource
    $this->_query = "SELECT * from $tablename"; //mysql query string

    $rs = $this->_conn->query( $this->_query );
    $total = $this->_conn->queryBuilder("SELECT count(*) as numberofRows FROM products");
    $this->_total = $total[0]->numberofRows;  //total number of rows
    ...
    public function getData( $limit = 10, $page = 1 ) { //set default argument values
        ...
        //echo $query;die;
        $rs = $this->_conn->queryBuilder( $query) or die($this->_conn->error);
